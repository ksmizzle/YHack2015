...
def main():
    ...
    with open('Fares.Fare-stg.csv', 'r') as csvfile:
        ...
        for row in csvreader:
            ...
            airportregion.setdefault(str(row[1]), ["Other", "Other"])
            airportregion.setdefault(str(row[2]), ["Other", "Other"])
            citypairdestinationtype.setdefault(str(row[1]+str(row[2])), [])
            # print(str(row[1])+str(row[2]) + " " + str(citypairdestinationtype[str(row[1])+str(row[2]) ]))
            result = db.flights.insert_one(
                {
                    "date": datetime.strptime(str(row[3]), r'%m/%d/%Y %H:%M'), #str(row[3]),
                    "dollarfare": float(row[6]),
                    "dollartax": float(row[7]),
                    "totalfare":float(row[6]) + float(row[7]),
                    "faretype": row[5],
                    "pointsfare":float(row[8]),
                    "pointstax":float(row[9]),
                    "destinationcode":str(row[2]),
                    "destinationgeographicregion":airportregion[str(row[2])][0],
                    "destinationmarketgroup":airportregion[str(row[2])][1],
                    "origincode":str(row[1]),
                    "origingeographicregion":airportregion[str(row[1])][0],
                    "originmarketgroup":airportregion[str(row[1])][1],
                    "domestic": domestic,
                    "destinationtype": citypairdestinationtype[str(row[1])+str(row[2]) ]
                })
